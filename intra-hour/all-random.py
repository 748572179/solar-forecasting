...
def run_forecast(target, horizon):
    ...
    for Xtra, Xtes, f in zip([train_X_endo, train_X], [test_X_endo, test_X], ['endo','exo']):
        ...
        for number, sample in samples:
            new_Xtra = Xtra.sample(n=sample,replace=False)
            indexes = new_Xtra.index
        # print(indexes)
            ...
            # new_train.insert(new_train.shape[1], "{}_{}_{}".format(target, name, f), train_pred)
                test.insert(test.shape[1], "{}_{}_{}_{}".format(target, name, f,sample), test_pred)
